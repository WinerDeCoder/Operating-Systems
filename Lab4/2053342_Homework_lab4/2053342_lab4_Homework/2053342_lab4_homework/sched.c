...
#define TIME_UNIT	95 // In microsecond
		...
		if (proc == NULL) {
			...
			*/
			...
			for (int i=0; i < exec_time; i++) {
				...
				count++;
				//printf("alo ");
				//printf(": %d pri ",get_priority(&ready_queue));
				usleep(TIME_UNIT);
				if (get_priority(&ready_queue) < proc->priority) {
					// If the process is a real-time process
					//timestamp++;
					//count++;
					//proc->burst_time)--;
					//usleep(TIME_UNIT);
				...
				if (proc->burst_time == 0) {
					/* If the process has finished its job
					 * then we free its PCB */
					 //usleep(TIME_UNIT);
					break;
				}
				else{
